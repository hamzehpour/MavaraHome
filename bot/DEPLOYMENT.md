...
> 📚 این راهنما در فاز بعدی به `docs/07-operations.md` منتقل می‌شود.
> مستندات کامل محصول: `docs/README.md`.
...
#   ENV=production
#   (API_ADMIN_TOKEN دیگر لازم نیست — هیچ اندپوینتی آن را نمی‌پذیرد؛ فقط برای سازگاری با .env های قدیمی خوانده می‌شود)
...
سپس در مرورگر واقعی:
1. از پنل ادمین سایت (`/pages/admin/`) یک رویداد جدید بسازید و آن را «در حال اجرا» یا «به‌زودی» کنید — باید بلافاصله (بدون ری‌استارت بات) هم در سایت عمومی، هم در لیست رویدادهای بات ظاهر شود؛ یعنی هر دو واقعاً به یک دیتابیس وصل‌اند.
2. روی همان رویداد در بات یک سانس بسازید و با `/admin` یک رزرو تستی بزنید — باید بلافاصله در `آمار/رزروها`ی پنل هم قابل دیدن باشد.

> **رزرو از سایت ساخته شده و فعال است** (این بند قبلاً خلافش را می‌گفت).
> بعد از دیپلوی، یک رزرو تستی مستقیم از خود سایت هم بزنید: صفحه‌ی رویداد ←
> انتخاب سانس ← فرم ← آپلود یک عکس به‌عنوان رسید ← بررسی اینکه در پنل
> «رزروها» ظاهر شود.
